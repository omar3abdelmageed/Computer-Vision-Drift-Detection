...
from core.local_files import resolve_dataset_root
from core.trusted_models import import_trusted_model
from core.types import SplitName, TaskType
from dashboard.components.status_cards import status_card
from database.repositories import Repository
from database.serialization import to_jsonable
from model_management.artifact_metadata import load_yolo_metadata
from model_management.baseline_builder import build_baseline_profile
from model_management.baseline_persistence import save_baseline_profiles
from model_management.compatibility import check_compatibility
from model_management.dataset_validation import validate_dataset
...
def render_model_registration_tab(client) -> None:
    ...
    dataset_paths = Repository(client, "dataset_paths")
...
def select_or_create_model(models: Repository, all_models: list[dict]) -> dict | None:
...
def render_create_model_form(models: Repository) -> None:
    st.divider()
...
def render_selected_model_metadata(models: Repository, selected: dict) -> None:
    st.divider()
...
def render_delete_model_dialog(models: Repository, selected: dict) -> None:
    ...
    if cancel_col.button("Cancel", width="stretch"):
        st.rerun()
    if delete_col.button("Delete model", icon=":material/delete:", type="primary", width="stretch"):
...
def render_task_type(models: Repository, selected_model: dict) -> str:
...
def render_dataset_registration(datasets: Repository, models: Repository, model_id: str, task_type: str) -> dict | None:
...
def render_model_registration(artifacts: Repository, models: Repository, model_id: str) -> dict | None:
    ...
    with st.form("register_model_path"):
        ...
        trusted_artifact = st.checkbox(
            "I trust this model artifact and its source",
            help="PyTorch model files must only be loaded from sources you trust.",
        )
        submitted = st.form_submit_button("Register model")
    if submitted:
        if not trusted_artifact:
            st.error("Confirm that you trust the model artifact before importing it.")
            return artifact_record
        try:
            trusted_model = import_trusted_model(artifact_path)
        except ValueError as exc:
            st.error(str(exc))
            return artifact_record
        local_path = trusted_model.path
        ...
        payload = {
            ...
            "artifact_sha256": trusted_model.sha256,
            "artifact_size_bytes": trusted_model.size_bytes,
...
def render_dataset_validation(datasets: Repository, dataset_paths: Repository, models: Repository, dataset_record: dict | None, task_type: str):
    ...
    st.dataframe([to_jsonable(split) for split in metadata.splits.values()], width="stretch")
    with st.expander("Validation issues"):
        st.json(to_jsonable(metadata.issues))
    return metadata


def persist_dataset_validation(datasets: Repository, dataset_paths: Repository, models: Repository, dataset_record: dict, metadata) -> bool:
    payload = {
...
def render_compatibility(artifacts: Repository, models: Repository, artifact_record: dict | None, dataset_metadata):
...
def persist_compatibility_result(artifacts: Repository, models: Repository, artifact_record: dict, result: dict) -> bool:
...
def render_baseline_report(baselines: Repository, model_id: str) -> None:
